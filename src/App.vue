    ...
    <div class="app-footer no-print">
      <div class="footer">
        ...
        <div class="footer-author">
          ...
          <span class="version-info" @click="handleRefreshApp" title="点击刷新应用">
            v{{ appVersion }}
          </span>
        </div>
      </div>
...
async function handleRefreshApp() {
  const confirmDialog = DialogPlugin.confirm({
    ...
    onConfirm: async () => {
      ...
      try {
        // 显示加载提示
        MessagePlugin.loading('正在刷新应用...', 0)
        ...
        // 3. 清除 localStorage 中的缓存数据（保留用户设置）
        const keysToRemove: string[] = []
        for (let i = 0; i < localStorage.length; i++) {
          const key = localStorage.key(i)
          if (key && (key.startsWith('weather_') || key.startsWith('diary_') || key.startsWith('cache_'))) {
        ...
        MessagePlugin.error('刷新失败，请手动刷新页面')
      }
    },
    onClose: () => {
      confirmDialog.hide()
